...
# # %%
#%%
# cats = sorted(glob.glob('/data4/kmtntoo/tutorial/data/subt/240918_SAAO/*cat'))

# newtbl = ascii.read(cats[0])
# newtblline = newtbl[newtbl['NUMBER']==256200]

# oldtbl = ascii.read(f'/data6/GECKO/S240915b/KMTN_SAAO/R/transients/{os.path.basename(cats[0])}')
    
# newtran = ascii.read(cats[1])
# newtranline = newtran[newtran['NUMBER']==256200]

# oldtran = ascii.read(f'/data6/GECKO/S240915b/KMTN_SAAO/R/transients/{os.path.basename(cats[1])}')

# newrb = ascii.read(cats[2])
# newrbline = newrb[newrb['NUMBER']==256200]

# oldrb = ascii.read(f'/data6/GECKO/S240915b/KMTN_SAAO/R/transients/{os.path.basename(cats[2])}')



#%%
# cats = sorted(glob.glob('/data4/kmtntoo/tutorial/data/subt/240918_SAAO/hd*cat'))
# old_dir = '/data6/GECKO/S240915b/KMTN_SAAO/R/transients/'
# a = ascii.read('/data4/kmtntoo/tutorial/data/subt/240918_SAAO/legacy/hdCalib.KMTNet_SAAO.S240915b.20240918-030255.R.960.stack.cat')
# for cat in cats:
#     # Read new and old tables
#     newtbl = ascii.read(cat)
#     oldtbl_path = os.path.join(old_dir, os.path.basename(cat))
#     oldtbl = ascii.read(oldtbl_path)
#     if 'flag_9' in newtbl.colnames:
#         newtbl.remove_column('flag_9')
#     # Check columns
#     if newtbl.colnames != oldtbl.colnames:
#         new_not_in_old = set(newtbl.colnames) - set(oldtbl.colnames)
#         print("Columns in newtbl but not in oldtbl:", new_not_in_old)

#         # Columns in oldtbl but not in newtbl
#         old_not_in_new = set(oldtbl.colnames) - set(newtbl.colnames)
#         print("Columns in oldtbl but not in newtbl:", old_not_in_new)
#         print(f"Column mismatch in {cat}")
#         # continue  # or handle as needed
    
#     # Extract the new row
#     new_row = newtbl[newtbl['NUMBER'] == 256200]
#     if len(new_row) == 0:
#         print(f"No row with NUMBER==256200 in {cat}")
#         # continue
    
#     # Append and write back
#     updated_tbl = vstack([oldtbl, new_row])
#     ascii.write(updated_tbl, oldtbl_path, overwrite=True)
#     print(f"Appended row to {oldtbl_path}")
